...
def print_help(argv):
    logger.info('Syntax: {} <package-name> <hivemq-package-dir> [artifact files ...]'.format(argv[0]))
    ...
    logger.info('In addition, environment variables named \'HIVEMQ_SOME_PARAMETER\' will be inserted against the provided package hivemq (with params of the form \'{{some-parameter}}\')')
...
def main(argv):
    ...
    # the package name:
    package_name = argv[1]
    # local path where the package hivemq is located:
