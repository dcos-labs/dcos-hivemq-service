...
$ %(prog)s kafka 1.2.3-4.5.6 \\
    /path/to/hivemq/jsons/ \\
    https://example.com/path/to/kafka-artifacts \\
    /path/to/artifact1.zip /path/to/artifact2.zip /path/to/artifact3.zip
"""

EPILOGUE_STRING = "In addition, environment variables named 'HIVEMQ_SOME_PARAMETER' " \
                "will be inserted against the provided package hivemq (with params of the " \
                " form '{{some-parameter}}')"
...
def main(argv):
    ...
    parser.add_argument('package_dir_path', type=str,
                        help='The local path where the package hivemq is located')
