...
class UniversePackageBuilder(object):
    ...
    def _get_hivemq_mapping_for_content(self, orig_content):
        '''Returns a hivemq mapping (dict) for the following cases:
        ...
        - Custom environment params like 'HIVEMQ_SOME_PARAM' which maps to '{{some-param}}'
        '''
        # default hivemq values (may be overridden via eg HIVEMQ_PACKAGE_VERSION envvars):
        now = time.time()
        hivemq_mapping = {
        ...
        # import any custom "HIVEMQ_SOME_PARAM" environment variables as "some-param":
        for env_key, env_val in os.environ.items():
            if env_key.startswith('HIVEMQ_'):
                # 'HIVEMQ_SOME_KEY' => 'some-key'
                hivemq_mapping[env_key[len('HIVEMQ_'):].lower().replace('_', '-')] = env_val

        sha_hivemq_maps = self._get_sha_hivemq_mapping(orig_content, hivemq_mapping)

        for key, value in sha_hivemq_maps.items():
            hivemq_mapping[key] = value

        return hivemq_mapping

    def _get_sha_hivemq_mapping(self, content: str, hivemq_mapping: dict) -> dict:
        """
        Look for any 'sha256:filename' or 'sha256:filename@url' hivemq params, and get shas for those.
        ...
        for sha_param in re.findall('"{{sha256:(.+?)}}"', content):
            sha_param_hivemqd = self._apply_hivemq_to_string(sha_param, hivemq_mapping)
            url_match = url_matcher.match(sha_param_hivemqd)
            if url_match:
                ...
                target_file_path = self._artifact_file_paths.get(sha_param_hivemqd, '')
                if not target_file_path:
                    raise Exception(
                        'Missing path for artifact file named \'{}\' (to calculate sha256). '.format(sha_param_hivemqd) +
                        'Please provide the full path to this artifact (known artifacts: {}), '.format(self._artifact_file_paths) +
                        'or specify a manifest URL (SHA256SUMS) with \'{}@<manifestURL>\''.format(sha_param_hivemqd))
                sha_value = self._calculate_sha256(target_file_path)

            # We add both the hivemqd and non-hivemqd sha patterns to the maps.
            sha_hivemq_maps['sha256:{}'.format(sha_param_hivemqd)] = sha_value
            sha_hivemq_maps['sha256:{}'.format(sha_param)] = sha_value

        return sha_hivemq_maps

    @staticmethod
    def _apply_hivemq_to_string(content: str, hivemq_mapping: dict) -> str:
        new_content = content
        prior_content = None
        while prior_content != new_content:
            prior_content = new_content
            for hivemq_key, hivemq_val in hivemq_mapping.items():
                new_content = new_content.replace('{{%s}}' % hivemq_key, hivemq_val)

        return new_content

    def _apply_templating_to_file(self, filename, orig_content):
        hivemq_mapping = self._get_hivemq_mapping_for_content(orig_content)
        new_content = self._apply_hivemq_to_string(orig_content, hivemq_mapping)
        ...
        hivemq_keys = list(hivemq_mapping.keys())
        hivemq_keys.sort()
        for key in hivemq_keys:
            logger.info('  {{%s}} => %s' % (key, hivemq_mapping[key]))
